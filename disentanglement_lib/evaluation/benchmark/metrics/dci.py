...
def _compute_dci(mus_train, ys_train, mus_test, ys_test, mus_eval, ys_eval, mode):
  ...
  if mode == "RF_class":
      ...
      params = [{"n_estimators": 10, "max_depth": d} for d in tree_depths]
      models = [RandomForestClassifier]*len(params)
      err_fn = utils.acc
  ...
  if mode == "RF_reg":
      tree_depths = range(2,11) 
      params = [{"n_estimators": 10, "max_depth": d} for d in tree_depths]
      models = [RandomForestRegressor]*len(params)
      err_fn = utils.nrmse
      importances_attr = 'feature_importances_'
      importances_fn = np.abs
...
#             models[a][i] = model
#             results[a, i] = np.mean( utils.nrmse(model.predict(x_eval.T), y_eval[i, :]) )
...
# ## Mode : RF_reg ##
# ###################
# def hyperparam_search_RF_reg(x_train, y_train, x_eval, y_eval, min_depth = 2, max_depth = 10):
#     num_factors = y_train.shape[0]
...
#         for i in range(num_factors):
#             model = RandomForestRegressor(n_estimators=10, max_depth = depth)
#             model.fit(x_train.T, y_train[i, :])
#             models[d][i] = model
#             results[d, i] = np.mean( utils.nrmse(model.predict(x_eval.T), y_eval[i, :]))
    
#     selected_ids = np.argmin(results, axis=0)
#     for f, i in enumerate(selected_ids):
#         selected_predictors[f] = models[i][f]
...
# ## Mode : gbt_classifier ##
# ###########################
# def compute_importance_RF_class(x_train, y_train, x_test, y_test, selected_predictors):
#   """Compute importance based on gradient boosted trees."""
#   num_factors = y_train.shape[0]
#   num_codes = x_train.shape[0]
#   importance_matrix = np.zeros(shape=[num_codes, num_factors],
...
# ## Mode : gbt_regressor ##
# ##########################
# def compute_importance_RF_reg(x_train, y_train, x_test, y_test, selected_predictors):
#   """Compute importance based on gradient boosted trees."""
#   num_factors = y_train.shape[0]
#   num_codes = x_train.shape[0]
#   importance_matrix = np.zeros(shape=[num_codes, num_factors],
...
#     model = selected_predictors[i]
#     importance_matrix[:, i] = np.abs(model.feature_importances_)
#     train_loss.append(np.mean(utils.nrmse(model.predict(x_train.T), y_train[i, :])))
#     test_loss.append(np.mean(utils.nrmse(model.predict(x_test.T), y_test[i, :])))
#   return importance_matrix, np.mean(train_loss), np.mean(test_loss)
