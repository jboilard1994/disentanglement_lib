...
def compute_rf_vae(dataholder,
    ...
    eval_votes = _generate_training_batch(dataholder, batch_size, num_eval, random_state, global_variances, active_dims)
...
@gin.configurable("rfvae_prune_dims", blacklist=["variances"])
...
def _generate_training_sample(dataholder,
    ...
    vary_index, possible_lock_vals = dataholder.sampling.sample_possible_factor_to_vary(batch_size, random_state)

    # Sample two mini batches of latent variables with same values at locked factor.
    factor, observationId = dataholder.sample_factors_with_locking_possibilities(1, random_state, vary_index, possible_lock_vals)
    factors.append(factor[0])
    observationIds.append(observationId[0])
  
    for i in range(batch_size-1):
        factor2, observationId2 = dataholder.sample_with_single_varied_factors(random_state, vary_index, factor)
    ...
    argmax = np.argmax(weighted_local_variances)
    return vary_index, argmax
